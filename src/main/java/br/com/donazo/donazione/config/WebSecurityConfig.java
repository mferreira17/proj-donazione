...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/javax.faces.resource/**", "/publica/**").permitAll()
				...
				.antMatchers("/autenticado").hasAnyRole("ADMINISTRADOR", "COLABORADOR").anyRequest().authenticated()
				.and().formLogin().loginPage("/index.xhtml").failureUrl("/index.xhtml?error=true").permitAll()
				.successForwardUrl("/autenticado/dashboard.xhtml")
				.defaultSuccessUrl("/autenticado/dashboard.xhtml", true).and().logout()
				.logoutSuccessUrl("/index.xhtml?logout=true").permitAll().and().rememberMe()
